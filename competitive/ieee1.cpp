...
long long tree[maxn],arr[maxn],lazy[maxn] ;
//build exact same
void build_tree(long long node,long long a,long long b){ //node is the index
...
void update_tree(long long node,long long a , long long b,long long i ,long long j,long long val){
...
//first if something needs to be updated from lazy update that!!
    ...
    long long q_left = query_tree(2*node,a,(a+b)/2,i,j) ;
    long long q_right = query_tree(2*node+1,(a+b)/2+1,b,i,j) ;
...
long long t ,n,x,y,z,i,j,k ,l,m,p,f,val ;
sld(n) ;
...
//            cin>>i>>j ;
    ...
    //}
          ...
          plldn(query_tree(1,0,n-1,0,n-1)) ;
